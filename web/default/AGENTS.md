...
## 视觉一致性（Geili Editorial）

任何 UI 改动都必须先读 [`DESIGN.md`](./DESIGN.md)，并遵守 `src/styles/theme-presets.css` 中的 `geili-editorial` 预设。提交前必须执行：
...
1. **一处朱砂**：每个视觉区域最多一个 `primary` 焦点，其余保持中性。
2. **hairline 分区**：优先用 1px `border-border` 与留白分区，不用阴影、玻璃、渐变堆叠。
3. **数字用衬线、标签用等宽**：大标题/大数字用 Fraunces，眉标/表头/状态/ID 用 IBM Plex Mono。
4. **留白优先**：宁可减少内容密度，也要保留标题、分区和操作之间的呼吸感。
5. **语义色克制**：状态使用低饱和 token 圆点或细描边，不使用大面积高饱和徽章。
...
- [一、项目概览](#一项目概览)
- [二、目录](#二目录)
- [视觉一致性（Geili Editorial）](#视觉一致性geili-editorial)
- [三、开发规范](#三开发规范)
  - [3.1 国际化](#31-国际化)
