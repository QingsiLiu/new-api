...
export function ApiKeyGroupCombobox({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        render={
          <Button
            ...
            aria-expanded={open}
            disabled={disabled}
            className='border-input bg-muted/40 hover:bg-muted/55 hover:text-foreground active:bg-background data-popup-open:border-ring data-popup-open:bg-background data-popup-open:ring-ring/20 h-auto min-h-14 w-full justify-between gap-2 rounded-[var(--radius-pill)] px-3 py-2 text-start shadow-none transition-[background-color,border-color,box-shadow] duration-150 data-popup-open:ring-[3px] sm:min-h-20 sm:gap-3 sm:px-4 sm:py-3'
          />
        }
      >
      ...
      <PopoverContent
        className='data-closed:zoom-out-100 data-open:zoom-in-100 data-[side=bottom]:slide-in-from-top-0 data-[side=left]:slide-in-from-right-0 data-[side=right]:slide-in-from-left-0 data-[side=top]:slide-in-from-bottom-0 border-border bg-popover w-[var(--anchor-width)] overflow-hidden rounded-[var(--radius-surface)] border p-0 shadow-none data-closed:duration-75 data-open:duration-100'
            ...
            <CommandGroup>
              {filteredOptions.map((option) => (
                <CommandItem
                  ...
                  className='data-[selected=true]:bg-accent items-start gap-3 rounded-[var(--radius-pill)] px-3 py-3 transition-colors'
                >
                  <Check
                    className={cn(
                      'mt-0.5 h-4 w-4',
