...
func TestComposeTieredTextQuotaKeepsToolCallSurcharges(t *testing.T) {
	...
	require.Equal(t, int64(2600), summary.ToolCallSurchargeQuota.Round(0).IntPart())
	require.Equal(t, 3600, quota)
}
...
func TestComposeTieredTextQuotaFallbackKeepsToolCallSurcharges(t *testing.T) {
	...
	require.Equal(t, int64(2500), summary.ToolCallSurchargeQuota.Round(0).IntPart())
	require.Equal(t, 3750, quota)
}
...
func TestCalculateTextQuotaSummaryFixedPriceAppliesImageCountOnceAndAllowsOverride(t *testing.T) {
	...
	summary := calculateTextQuotaSummary(ctx, relayInfo, usage)
	// Geili CNY 体系下 QuotaPerUnit=CNYQuotaUnit（非上游 500k），期望值按单位推导
	require.Equal(t, int(0.12*common.QuotaPerUnit*3), summary.Quota)
	...
	summary = calculateTextQuotaSummary(ctx, relayInfo, usage)
	require.Equal(t, int(0.12*common.QuotaPerUnit*2), summary.Quota)
}
