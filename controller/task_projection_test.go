...
func TestTasksToDtoProjectsCreditsLifecycle(t *testing.T) {
	...
	require.NotNil(t, items[0].Credits)
	require.Equal(t, "2", *items[0].Credits)
	require.NotNil(t, items[0].QuotaPerCredit)
	...
	require.NotNil(t, items[2].ReservedCredits)
...
func TestTasksToDtoKeepsCreditsFieldsOffByDefault(t *testing.T) {
	...
	require.Nil(t, item.Credits)
	require.Nil(t, item.QuotaPerCredit)
