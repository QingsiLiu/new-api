...
func GetStatus(c *gin.Context) {
	if _, err := service.LoadGeiliFunnelConfig(); err != nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"success": false,
...
func SendPasswordResetEmail(c *gin.Context) {
	...
	c.Header("Link", `</api/reset_password>; rel="successor-version"`)
	sendPasswordResetEmail(c, model.NormalizeEmail(c.Query("email")))
}
...
func SendPasswordResetEmailPost(c *gin.Context) {
	setPasswordResetNoStore(c)
	...
	sendPasswordResetEmail(c, model.NormalizeEmail(req.Email))
}

var sendPasswordResetMail = common.SendEmail

func sendPasswordResetEmail(c *gin.Context, email string) {
	if err := common.Validate.Var(email, "required,email"); err != nil {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	if _, err := model.GetUniqueUserByEmail(email); err == nil {
		...
		if passwordResetV2Enabled() {
			credential, err = issuePasswordResetTicket(email)
		} else {
			credential = common.GenerateVerificationCode(0)
			common.RegisterVerificationCodeWithKey(email, credential, common.PasswordResetPurpose)
		}
		if err != nil {
			logAuthSecurityEvent(c.Request.Context(), authLogError, "password_reset_ticket_issue_failed", authSecurityFields{
				Method: "password_reset_v2", Email: email, Err: err,
			})
			c.JSON(http.StatusOK, gin.H{"success": true, "message": ""})
			return
		}
		link := html.EscapeString(passwordResetLink(email, credential))
		subject := fmt.Sprintf("%s密码重置", common.SystemName)
		content := fmt.Sprintf("<p>您好，你正在进行%s密码重置。</p>"+
			"<p>点击 <a href='%s'>此处</a> 进行密码重置。</p>"+
			"<p>如果链接无法点击，请尝试点击下面的链接或将其复制到浏览器中打开：<br> %s </p>"+
			"<p>重置链接 %d 分钟内有效，如果不是本人操作，请忽略。</p>", common.SystemName, link, link, common.VerificationValidMinutes)
		err := sendPasswordResetMail(subject, email, content)
		if err != nil {
