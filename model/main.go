...
func migrateDB() error {
	...
	if err := migrateGeiliChannelTypeRenumber(); err != nil {
		return err
	}
	return nil
}

// migrateGeiliChannelTypeRenumber 一次性把 Geili 自有渠道类型从 58/59/60
// 迁到 101/102/103（58 与上游 AdvancedCustom 撞号）。用 option 标记防重入：
// 迁移只在标记不存在时执行一次，之后 58 即为上游 AdvancedCustom 语义。
func migrateGeiliChannelTypeRenumber() error {
	const doneKey = "GeiliChannelTypeRenumbered"
	var opt Option
	if err := DB.Where("key = ?", doneKey).First(&opt).Error; err == nil {
		return nil // 已迁移
	}
	renumber := map[int]int{58: 101, 59: 102, 60: 103}
	for from, to := range renumber {
		if err := DB.Model(&Channel{}).Where("type = ?", from).Update("type", to).Error; err != nil {
			return err
		}
		// tasks.platform 存的是渠道类型数字字符串（见 relay.GetTaskPlatform），同步迁移
		if err := DB.Model(&Task{}).Where("platform = ?", strconv.Itoa(from)).Update("platform", strconv.Itoa(to)).Error; err != nil {
			return err
		}
	}
	return DB.Create(&Option{Key: doneKey, Value: "true"}).Error
}
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&QuotaData{}, "QuotaData"},
		{&Task{}, "Task"},
		{&AsyncTaskAttempt{}, "AsyncTaskAttempt"},
		{&Model{}, "Model"},
		{&Vendor{}, "Vendor"},
		...
		{&UserQuotaChangeRecord{}, "UserQuotaChangeRecord"},
