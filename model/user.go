...
type User struct {
	...
	SignupCreditSource   string                     `json:"-" gorm:"-:all"`
	SignupCreditIdentity string                     `json:"-" gorm:"-:all"`
}
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	if float64(quota) < common.QuotaPerUnit {
		return fmt.Errorf("转移金额最小为%s！", logger.LogQuota(int(common.QuotaPerUnit)))
	}
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			user.Quota = initialQuotaForNewUser()
			...
			if err := grantSignupCreditsWithTx(tx, user); errors.Is(err, ErrCreditGrantIdentityUsed) {
				return nil
			} else {
				return err
			}
...
func (user *User) finishInsert(inviterId int) {
	...
	if !common.CreditsV1Enabled() && common.QuotaForNewUser > 0 {
...
func initialQuotaForNewUser() int {
